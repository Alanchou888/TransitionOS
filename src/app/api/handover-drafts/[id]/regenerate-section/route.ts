...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const current = await prisma.handoverDraft.findUnique({
  ...
  const next = await prisma.handoverDraft.create({
    data: {
      transitionTaskId: current.transitionTaskId,
      version: nextVersion,
      contentMarkdown: toMarkdown(`Handover Draft v${nextVersion}`, regenerated.generated),
  ...
  await createAuditLog({
    ...
    action: "HANDOVER_SECTION_REGENERATED",
    details: {
      sectionKey: parsed.data.sectionKey,
